...
def create_simple_pdf_response(filled_responses: Dict[str, str]) -> str:
    ...
    try:
        ...
        # Ultimate fallback - create empty PDF manually
        return create_minimal_pdf_fallback(filled_responses)

def create_minimal_pdf_fallback(filled_responses: Dict[str, str]) -> str:
    """
    Create a minimal valid PDF structure as last resort
    """
    # Create minimal PDF structure manually
    pdf_content = """%PDF-1.4
1 0 obj
<<
/Type /Catalog
...
/F1 12 Tf
100 700 Td
(FILLED GRANT APPLICATION) Tj
0 -30 Td
(Generated by GrantSeeker) Tj
ET
endstream
endobj
    ...
    try:
        ...
        try:
            ...
            if not pdf_success:
                ...
                logging.info("Falling back to create_simple_pdf_response")
                filled_pdf_data = create_simple_pdf_response(filled_responses)
                pdf_success = True
                fill_method = "Simple_PDF_Fallback"
                logging.info("Simple PDF fallback completed")
                
        except Exception as e:
            logging.error(f"Error generating PDF: {str(e)}")
            # Fallback to simple text-based representation
            try:
                logging.info("Main PDF generation failed, trying simple PDF fallback")
                filled_pdf_data = create_simple_pdf_response(filled_responses)
                pdf_success = True
                fill_method = "Error_Fallback"
                logging.info("Error fallback PDF generation completed")
